...
def generate_response(messages, question, username):
    try:
        ...
        # Use intelligent keyword-based context from today's log
        keywords = extract_keywords(question)
        LOGGER.debug(f"Extracted keywords: {keywords}")
        
        if keywords:
            recent_context = get_relevant_context(keywords, max_lines=200)
            LOGGER.debug(f"Using keyword-based context: {len(recent_context)} characters")
        else:
            # No meaningful keywords, use today's context
            ...
            if todays_context:
                ...
                with open(LOG_FILE, "r", encoding='utf-8') as f:
                    ...
                    ][:200]  # Limit to 200 messages to avoid payload issues
                    recent_context = "\n".join(recent_context)
        ...
        # Build prompt with reasonable limits to avoid payload issues
        ...
        if messages:
            ...
            prompt += messages[-5000:] + "\n" if len(messages) > 5000 else messages + "\n"
        prompt += "Kysymys: " + question
        
        # Final safety check - if prompt is too large, reduce context instead of truncating
        if len(prompt) > 40000:
            # Reduce recent_context to fit within limits
            target_context_size = 30000
            if len(recent_context) > target_context_size:
                ...
                for line in reversed(context_lines):
                    if current_size + len(line) > target_context_size:
                        break
                    reduced_lines.insert(0, line)
                    current_size += len(line)
                
                recent_context = '\n'.join(reduced_lines)
                LOGGER.debug(f"Context reduced to {len(recent_context)} characters to avoid payload issues")
